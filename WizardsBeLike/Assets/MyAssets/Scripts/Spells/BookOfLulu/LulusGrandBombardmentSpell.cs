...
[CreateAssetMenu(fileName = "New LulusGrandBombardmentSpell", menuName = "Spells/LulusGrandBombardmentSpell")]
public class LulusGrandBombardmentSpell : Spell, ISuperSpell
{
    ...
    public float timeScaleDuringCast = 0.5f; // Time scale to slow down the game
    ...
    private IEnumerator CastSequence(Transform castingPoint, PlayerMovement playerMovement, MagicalWeapon magicalWeapon, float currentCastingPower)
    {
        ...
        // Original casting logic: summon Lulu, drop the bomb, etc.
    ...
    {
        yield return new WaitForSeconds(bombReleaseTime * Time.timeScale);
         // Check if Lulu is still in the game (not destroyed)
        if (luluRb != null)
        {
            // Define an offset for where the bomb will be instantiated
            Vector3 bombOffset = new Vector3(0, -2f, 0); // Adjust the y-value as needed to avoid collision with Lulu

            // Instantiate the feces bomb at an offset from Lulu's current position to prevent collision
            GameObject fecesBombInstance = Instantiate(fecesBombPrefab, luluRb.position + bombOffset, Quaternion.identity);
